    ...
    def __init__(self, conf_file=None, init_conf=None):
        ...
        try:
            ...
            for sample in [i for i in cp.sections() if i.startswith("sample")]:
                ...
                if self.idx2:
                    Sample(name=cp.get(sample, "name"),
                           index=(cp.get(sample, "index1_seq") + cp.get(sample, "index2_seq")))
                else:
            ...
            try:
                while True:
                    ...
                    index = index1[self.idx1_pos["start"]:self.idx1_pos["end"]] + index2[
                                                                                  self.idx2_pos["start"]:self.idx2_pos[
                                                                                      "end"]]
                    molecular = index1[self.mol1_pos["start"]:self.mol1_pos["end"]] + index2[self.mol2_pos["start"]:
                                                                                             self.mol2_pos["end"]]
